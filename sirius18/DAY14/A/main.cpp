...
void pull(int v){
    //cerr << v;
    if (st[2*v].x > st[2*v+1].x || ((st[2*v].x == st[2*v+1].x  && st[2*v].ind < st[2*v+1].ind))){
        st[v].x = st[2*v].x;
        st[v].ind = st[2*v].ind;
        return;
    }
    st[v].x = st[2*v+1].x;
    st[v].ind = st[2*v+1].ind;
}
...
pair<int, int> getMax(int v, int l, int r, int tl, int tr){
    ...
    if (v1.ff > v2.ff || ((v1.ff == v2.ff) && v1.ss < v2.ss)){
        return v1;
    }
    return v2;
}
...
int main()
{
    ...
    for (int i=0; i<k;i++){
        ...
        cout << getMax(1, l, r+1, 1, n+1).ss + 1 << ' ';
    }
