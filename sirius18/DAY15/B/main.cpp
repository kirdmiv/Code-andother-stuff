...
int check(int i){
    int d = s.size();
    ull f1 = (((sq1 + h1[0][d] - (h1[0][i] * bpow[0][d])) % p1 + (h1[0][i] * bpow[0][d-i]) % p1)) % p1;
    ull f2 = (((sq2 + h1[1][d] - (h1[1][i] * bpow[1][d])) % p2 + (h1[1][i] * bpow[1][d-i]) % p2)) % p2;

    cout << f1 << ' ' << h2[0][d] << '\n';
    return f1 == h2[0][d] && f2 == h2[1][d];
...
int main()
{
    ...
    for(int i = 1; i<t.size()+1; ++i){
            h2[0][i] = (h2[0][i-1] * b + t[i-1]) % p1;
            h2[1][i] = (h2[1][i-1] * b + t[i-1]) % p2;
            //cout << h2[0][i] << '\n';
    }
